 ...
 * APLogger Class File
 *
 * Contains the class definition for the APLogger, a final class for 
...
/**
 * APLogger is responsible for handling all system logging for the eGloo web 
...
final class APLogger {
     ...
     public static function setLoggingType($type){
     	...
     	if($type == self::$LOG_HTML){
     		...
     		if ( (file_put_contents( self::$logFilePath, $header . "\n", FILE_APPEND ) ) === false ) {
           		throw new APLoggerException( 'Error writing to log' );
           	}
     	}
     ...
     * @throws APLoggerException    if there was an error writing to the log file
     * @returns NULL 
     */
...
/**
 * Private exception subclass for use by APLogger
 */
final class APLoggerException extends Exception {

   /**
    * APLoggerException constructor.  Takes a message and a code and invokes
