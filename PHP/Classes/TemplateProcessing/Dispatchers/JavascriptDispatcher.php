...
class JavascriptDispatcher extends TemplateDispatcher {
	...
	public static function getInstance( $application, $interfaceBundle ) {
		if ( !isset(self::$singletonDispatcher) ) {
			...
			if ( (self::$singletonDispatcher = $dispatchCacheRegionHandler->getObject(	eGlooConfiguration::getUniqueInstanceIdentifier() . '::' . 'JavascriptDispatcherNodes', 'Dispatching', true ) ) == null ) {
				...
				$dispatchCacheRegionHandler->storeObject( eGlooConfiguration::getUniqueInstanceIdentifier() . '::' . 'JavascriptDispatcherNodes', self::$singletonDispatcher, 'Dispatching', 0, true );
			} else {
