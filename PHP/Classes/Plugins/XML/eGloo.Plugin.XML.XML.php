...
/**
 * eGloo\Plugin\XML\XML Class File
 *
 * Contains the class definition for the eGloo\Plugin\XML\XML
 * 
...
class XML {
	...
	public function __construct( $xml ) {
		if ( is_string($xml) ) {
			...
			throw new Exception( 'eGloo\Plugin\XML\XML does not support instantiation from type ' . gettype($xml) . '.' );
		}

		if ( !$this->_simpleXMLObject ) {
			$error_message = 'eGloo\Plugin\XML\XML: simplexml_load_file( "' . $xml . '" ): ' . var_export(libxml_get_errors(), true);
			Logger::writeLog( Logger::EMERGENCY, $error_message );
					...
					if ( $build_flags & self::BUILD_STRUCTURE ) {
						...
						try {
							if ( $child !== false && !empty($child)) {
								$childObjWrapper = new self( $child );
							}
						} catch ( Exception $e ) {
							// Ignore, logged inside constructor
						}
	 ...
	 */
	public function setChild( $xpath, Child $child ) {
	...
	/**
	 * Get eGloo\Plugin\XML\XML object as array
	 *
	 * @return array Array representation of this eGloo\Plugin\XML\XML
	 */
